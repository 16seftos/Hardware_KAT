 ...
 %{\fontsize{12pt}{14pt}\selectfont
    ...
    \begin{align*}
        ...
        \interp{ a + b }(S)
            ...
            & \textcolor{red}{ or\ \ \interp{a\cup b}(S) ?}\\ % probably the best option?
        %Product
        \interp { a \cdot b }(S)
             \triangleq\ &
             \mathsf{let}\ S_a = \interp { a }(S) \\
        ...
        %Policy Union
         \interp { p + q }(S)
            ...
            \\
            & \textcolor{red}{ or\ \ S_p \cup S_q ?}\\%better option?
        %Policy Concatnation
        \interp { p \cdot q }(S)
            \triangleq\ &
            \interp{q}(\interp{p}(S))\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %Definitions table
    \begin{tabular}{l r c l}
        Locations     & $L$     & $::=$     & $\mathsf{INSTR_{ADDR}\ |\ R(s/t/d)_{ADDR}\ |\ MEM_{ADDR}}$\\
        Values        & $V$     & $::=$     & $\mathsf{R(s/t/d)_{VAL}\ |\ ALU(a/b/imm)_{IN}\ |\ ALU_{OUT}\ |\ MEM_{OUT}}$\\
        Types         & $T$     & $::=$     & $\text{MIPS64 instruction set}$ \\
        Branches      & $B$     & $::=$     & $\mathsf{FROM\ |\ TO}$ \\
        %Branch values are for the branch checker
        MemAddrLocs   & $M_{a}$ & $::=$     & $\mathsf{MEM_{ADDR}}$\\
        %MemAddrLocs are for the pre-mem checker
        \\
        Bitwise Op    & $<b>$   & $::=$     & $\mathsf{AND\ |\ OR\ |\ XOR\ |\ NOT}$\\
        Range Op      & $<r>$   & $::=$     & $\mathsf{<\ |\ \le\ |\ =\ |\ \ge\ |\ >\ |\ \not=}$\\
    \end{tabular}\\
    \\
    ...
    \begin{tabular}{l c r l l}
        Predicates  & $a,b$  & $::=$  & $0$             & \textit{False}        \\
                    &        & $\mid$ & $1$             & \textit{Identity}     \\
                    &        & $\mid$ & $L | V <r> n$   & \textit{Range}        \\  
                    &        & $\mid$ & $L | V <b> n$   & \textit{BitOp}        \\  
                    &        & $\mid$ & $T \in n$       & \textit{Instr Element}\\  
                    &        & $\mid$ & $B <r> n$       & \textit{Branch Range} \\  
                    &        & $\mid$ & $M <r> n$       & \textit{PreMEM Range} \\
                    %
                    &           & $\mid$ & $a + b$      & \textit{Sum}          \\
                    &           & $\mid$ & $a \cdot b$  & \textit{Product}      \\
                    &           & $\mid$ & $\neg \ a$   & \textit{Negation}
    ...
    \begin{tabular}{l c r l l}
        Policies    & $p,q$ & $::=$  & $a$              & \textit{Test}             \\
                    &       & $\mid$ & $\mathsf{OUT}\{V\}$ & \textit{Bind Output}   \\ 
                    &       & $\mid$ & $\mathsf{PAUSE\{Is\}}$ & \textit{Pause}      \\
                    %to calculate other data
                    &       & $\mid$ & $p + q$          & \textit{NonDeterministic Choice}   \\ % uses nondeterministic FST
                    &       & $\mid$ & $p \cdot q$      & \textit{Sequential Concatenation} \\ % uses stack
                    &       & $\mid$ & $p ^{*}$         & \textit{Kleene Star} % uses stack
    \end{tabular}\\
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% END OF SECTION 4             %%%
%%% SYNTAX INTERMEDIATE          %%%
